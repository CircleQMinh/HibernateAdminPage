...
import Model.Customer;import Model.Employee;import Model.Admin;
...
public class AccountDao {
     ...
     public Account getAccountByUsername(String username) // all account 
    {
        ...
        try {
            ...
            System.out.println("NoResult CustomerDAO getAccCusbyUsername exception");
            if (transaction != null) {
                transaction.rollback();
                session.close();
                sessionFactory.close();
    ...
    public Object getAccountInformation(Account acc) {
        ...
        try {
            ...
            else if(role.equals("customer"))
            {
                ...
                System.out.println("tren query");
                Query<Customer> query = session.createQuery(hql,Customer.class);
                System.out.println("uder query");
                query.setParameter("username", acc.getUsername());
                Customer cus = query.uniqueResult();
                System.out.println("uder cus");
                transaction.commit();session.close();sessionFactory.close();                
                return cus;          
            }
        } catch (Exception e) {
